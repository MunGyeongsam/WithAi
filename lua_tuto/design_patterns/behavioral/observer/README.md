...
한 객체의 변화를 여러 수신자에게 방송할 때 Observer가 유용합니다.

- **체력/점수 변경 알림**: 플레이어 체력이나 점수가 바뀌면 UI 게이지·효과음·업적 시스템이 동시에 반응하되, 발행자는 수신자를 알 필요가 없습니다.
- **이벤트 버스**: "적 처치", "아이템 획득" 같은 게임 이벤트를 발행하면 관심 있는 시스템만 구독해 반응합니다.
- **퀘스트 진행 감시**: 적 처치·아이템 수집 이벤트를 퀘스트 시스템이 구독해 목표 달성률을 갱신합니다.
- **설정 변경 브로드캐스트**: 볼륨·해상도 변경을 구독 중인 오디오·렌더 시스템에 알립니다.
