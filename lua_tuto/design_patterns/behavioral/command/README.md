...
요청을 값으로 만들어 큐잉·기록·취소할 때 Command가 유용합니다.

- **입력 리바인딩과 리플레이**: 입력을 바로 처리하지 않고 `JumpCommand` 같은 객체로 바꾸면, 키 재배치·리플레이 기록·AI 재사용이 모두 가능해집니다.
- **턴제 행동 큐잉**: 유닛의 이동·공격 명령을 큐에 넣었다가 턴이 실행될 때 순서대로 처리하고, 취소 명령으로 되돌립니다.
- **매크로/콤보 기록·재생**: 연속 입력을 커맨드 리스트로 기록해 나중에 재생하거나 콤보로 묶습니다.
- **네트워크 액션 직렬화**: 플레이어 액션을 커맨드 객체로 만들어 직렬화하고 서버로 전송합니다.
