...
실제 게임을 만들 때 Singleton이 자연스럽게 쓰이는 상황입니다.

- **전역 오디오 매니저**: BGM과 효과음의 볼륨·뮤트 상태를 하나의 인스턴스에서 관리해, 어떤 씬에서 재생을 요청해도 같은 설정이 적용됩니다.
- **씬 간 공유 GameState/세이브 슬롯**: 메뉴·플레이·결과 화면이 같은 진행 상태(점수, 스테이지, 소지 골드)를 하나의 저장소에서 읽고 씁니다.
- **리소스 캐시**: 스프라이트·폰트·사운드를 한 번만 로드해 캐시하고, 여러 시스템이 같은 핸들을 재사용해 중복 로드와 메모리 낭비를 막습니다.
- **로그·메트릭 수집기**: FPS, 처치 수, 오류 로그를 한 곳에 모아 디버깅과 밸런싱에 활용합니다.
