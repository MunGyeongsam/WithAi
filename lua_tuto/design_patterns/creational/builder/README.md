...
선택 항목이 많은 복잡한 객체를 단계별로 조립할 때 Builder가 적합합니다.

- **적 스폰 구성 빌더**: 체력·이동 속도·AI 종류·드롭 아이템·저항을 단계별로 지정해 다양한 적을 만들되, 지정하지 않은 값은 기본값으로 채웁니다.
- **파티클 이펙트 프리셋**: 색·수명·방출률·중력·블렌드 모드를 조립해 폭발·연기·불꽃 프리셋을 재사용 가능한 형태로 구성합니다.
- **스테이지/웨이브 빌더**: 웨이브 수, 스폰 지점, 등장 적 종류, 간격을 단계적으로 쌓아 레벨 데이터를 구성합니다.
- **캐릭터 장비 조합**: 무기·방어구·액세서리·스킬을 순서대로 붙여 최종 캐릭터 스탯을 계산합니다.
