...
복잡한 서브시스템을 단순한 창구로 감쌀 때 Facade가 유용합니다.

- **GameServices 창구**: 오디오·파티클·저장·입력 서브시스템을 하나의 파사드로 묶어, 게임플레이 코드가 `services.play("hit")`처럼 간단히 호출합니다.
- **레벨 로딩**: 에셋 로드·맵 파싱·엔티티 배치·BGM 시작을 `loadLevel(id)` 하나로 감싸, 씨 전환 코드가 내부 단계를 몰라도 되게 합니다.
- **네트워크/매치메이킹**: 소켓·직렬화·재접속 로직을 파사드로 숨겨 `matchmaking.findGame()` 같은 단순 호출만 노출합니다.
- **물리 초기화**: 월드 생성·충돌 그룹·중력 설정을 하나의 초기화 함수로 묶습니다.
