...
부분과 전체를 같은 방식으로 다루는 계층 구조에 Composite가 유용합니다.

- **UI 트리**: 패널 안에 버튼·라벨·하위 패널이 들어있을 때, 루트 패널에 `draw`/`update`를 한 번 호출하면 자식 위젯까지 재귀적으로 처리됩니다.
- **씬 그래프/엔티티 계층**: 부모 오브젝트의 변환(위치·회전)이 자식에 누적되는 구조로 보스와 팔·무기를 함께 이동시킵니다.
- **스킬 트리/퀵스트 목표**: 상위 스킬이 하위 스킬을, 퀵스트가 하위 목표를 포함하는 구조를 재귀로 순회해 완료 여부를 집계합니다.
- **인벤토리 폴더**: 폴더 안에 아이템과 하위 폴더를 두어 전체 무게·개수를 동일한 인터페이스로 계산합니다.
