...
반복되는 데이터를 공유해 메모리를 아낀 때 Flyweight가 유용합니다.

- **타일맵**: 수천 개의 타일이 각자 이미지를 들지 않고, 타일 종류별 원본(퀴드·재질)을 공유하고 위치만 개별로 가집니다.
- **파티클·발사체**: 수백 개의 총알·파티클이 스프라이트와 메쉬 같은 불변 데이터를 공유하고, 위치·속도·수명만 개별 상태로 둘니다.
